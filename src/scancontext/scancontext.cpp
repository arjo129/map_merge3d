...
void SCManager::makeAndSaveScancontextAndKeys(std_msgs::Header header, pcl::PointCloud<SCPointType> & _scan_down )
{
    ...
    polarcontexts_timestamp_.push_back(header);

    // cout <<polarcontext_vkeys_.size() << endl;

} // SCManager::makeAndSaveScancontextAndKeys

map_merge3d::ScanContextDescriptor SCManager::getScanContext(pcl::PointCloud<SCPointType> & _scan_down )
{
    Eigen::MatrixXd sc = makeScancontext(_scan_down); // v1 
    Eigen::MatrixXd ringkey = makeRingkeyFromScancontext( sc );
    Eigen::MatrixXd sectorkey = makeSectorkeyFromScancontext( sc );
    ...
    map_merge3d::ScanContextDescriptor desc;
    desc.inv_key = serializeMatrix(ringkey);
    desc.polar_context = serializeMatrix(sc);
    desc.vkeys = serializeMatrix(sectorkey);
...
std::pair<int, float> SCManager::detectLoopClosureID ( map_merge3d::ScanContextDescriptor desc ) {
    int loop_id { -1 }; // init with -1, -1 means no loop (== LeGO-LOAM's variable "closestHistoryFrameID")

    auto curr_key = eig2stdvec( deserializeMatrix(desc.inv_key) ); // current observation (query)
    auto curr_desc = deserializeMatrix(desc.polar_context); // current observation (query)
...
void SCManager::addLoopClosureDescriptor(map_merge3d::ScanContextDescriptor desc) {
    polarcontexts_.push_back( deserializeMatrix(desc.polar_context) ); 
    polarcontext_invkeys_.push_back( deserializeMatrix(desc.inv_key) );
    polarcontext_vkeys_.push_back( deserializeMatrix(desc.vkeys) );
    polarcontext_invkeys_mat_.push_back( eig2stdvec( deserializeMatrix(desc.inv_key) ) );
    polarcontexts_timestamp_.push_back(desc.header);
}
size_t SCManager::get_number_of_entries(){
    return polarcontexts_.size();
}

ros::Time SCManager::getTimeOfId(size_t idx) {
    if(idx > polarcontexts_.size()) {
        throw IndexDoesNotExistException(idx, polarcontexts_.size());
    }
    return polarcontexts_timestamp_[idx].stamp;
}

map_merge3d::ScanContextDescriptor SCManager::getDescriptorId(size_t idx) {
    map_merge3d::ScanContextDescriptor desc;
    if(idx > polarcontexts_.size()) {
        throw IndexDoesNotExistException(idx, polarcontexts_.size());
    }
    desc.polar_context = serializeMatrix(polarcontexts_[idx]);
    desc.inv_key = serializeMatrix(polarcontext_invkeys_[idx]);
    desc.vkeys = serializeMatrix(polarcontext_vkeys_[idx]);
    
    desc.header = polarcontexts_timestamp_[idx];
    return desc;
}
// } // namespace SC2